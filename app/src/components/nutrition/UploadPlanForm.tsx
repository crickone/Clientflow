...
export function UploadPlanForm({ initial }: { initial: PlanInput }) {
  const router = useRouter();
  const fileRef = useRef<HTMLInputElement>(null);
  const [title, setTitle] = useState(initial.title === "New Nutrition Plan" ? "" : initial.title);
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      fd.set("file", file);
      const res = await fetch("/api/nutrition/upload", { method: "POST", body: fd });
  ...
  const save = () => {
    if (!title.trim()) return toast.error("Enter a plan title.");
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <h1 style={{ margin: 0, fontFamily: "var(--font-heading), sans-serif", fontSize: 24, textTransform: "uppercase" }}>
          {initial.id ? "Edit uploaded plan" : "Upload Nutrition Plan"}
        </h1>
      </div>
        ...
        <div>
          ...
          {filename ? (
            ...
            >
              ...
              <div style={{ fontSize: 13.5, color: "var(--accent-ink)" }}>
                {uploading ? "Uploading…" : "Drop your file here or click to upload."}
              </div>
              <div style={{ fontSize: 12, color: "var(--text-tertiary)" }}>Only .pdf, .xls, .xlsx, .doc, .docx files can be uploaded</div>
            </button>
          )}
        </div>

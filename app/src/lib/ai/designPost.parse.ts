...
HOW MUCH FITS ON ONE SLIDE. A slide is a 1080x1080 square read at thumbnail size in a feed, not a page. A finished slide's markup runs to roughly 1,000-1,800 characters: an eyebrow, a heading, and one short paragraph -- sixty to eighty words of body copy at the absolute most, usually fewer. TEACHING SOMETHING DOES NOT MEAN WRITING MORE. It means the sixty words you have say something worth knowing rather than describing the room. An explanation that will not fit in that is not a longer slide -- it is the next slide, or a different post. A slide that runs long does not get read; it gets rendered with its last line sliced off at the canvas edge.

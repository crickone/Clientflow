...
// (`/api/agents/[key]/chat`). The route runs `runAgentTurn` in a detached
// server-side loop that is NOT killed by a client disconnect (refresh/
// navigate/close) — this table is what makes that survivable: progress
// (`text`) is persisted as it streams, and the terminal outcome (`status` +
// `pending`/`artifacts`/`error`) is written once the loop ends, so a
// reconnecting client (DR2) can pick the run back up instead of losing it.
// `pending`/`artifacts` are JSON-serialised arrays (`PendingWrite[]` from
// @/lib/agents/runAgentTurn / `ToolArtifact[]` from @/lib/assistant/tools) —
// see @/lib/agents/runStore.ts, the only code that reads/writes this table.
export const agentRuns = sqliteTable("agent_runs", {
  id: text("id").primaryKey(), // crypto.randomUUID(), generated by runStore.createRun
  conversationId: text("conversation_id").notNull(),
  agentKey: text("agent_key").notNull(),
  model: text("model").notNull(),
  status: text("status", { enum: ["running", "awaiting_approval", "done", "error"] })
    .notNull()
    .default("running"),
  text: text("text").notNull().default(""), // accumulated assistant text so far
  pending: text("pending"), // JSON PendingWrite[] — set when status = 'awaiting_approval'
  artifacts: text("artifacts"), // JSON ToolArtifact[]
  error: text("error"),
  createdAt: integer("created_at", { mode: "timestamp_ms" }).notNull().default(sql`(unixepoch() * 1000)`),
  updatedAt: integer("updated_at", { mode: "timestamp_ms" }).notNull().default(sql`(unixepoch() * 1000)`),
});

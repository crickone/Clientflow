...
import { deleteWorkout, duplicateWorkout, saveWorkout } from "@/lib/individualWorkouts";
...
const schema = z.object({
  id: z.number().int().positive().optional(),
  name: z.string().trim().min(1, "Give the workout a name.").max(200),
...
export async function saveWorkoutAction(raw: unknown): Promise<SaveResult> {
  await requireUser();
  const parsed = schema.safeParse(raw);
  if (!parsed.success) return { ok: false, error: parsed.error.issues[0]?.message ?? "Invalid workout." };
  const id = saveWorkout(parsed.data as WorkoutInput);
  revalidatePath("/workout/workouts");
  revalidatePath(`/workout/workouts/${id}`);
  return { ok: true, id };
}
...
export async function deleteWorkoutAction(id: number) {
  ...
  if (!p.success) return;
  deleteWorkout(p.data);
  revalidatePath("/workout/workouts");
}

export async function duplicateWorkoutAction(id: number): Promise<SaveResult> {
  await requireUser();
  const p = idSchema.safeParse(id);
  if (!p.success) return { ok: false, error: "Invalid id." };
  const newId = duplicateWorkout(p.data);
  if (!newId) return { ok: false, error: "Workout not found." };
  revalidatePath("/workout/workouts");

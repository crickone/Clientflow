 ...
 *
 * The actual guards (tenant-scope, validate/normalize, no-op, uniqueness,
 * update) live in applyMemberEmailUpdate (@/lib/users/emailUpdate) — a plain,
 * non-"use server" module, so it can never itself become a callable Server
 * Action endpoint. This wrapper is the only intended caller: it resolves
 * `tenantId` from the authenticated admin's own session (never from caller

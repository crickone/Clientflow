...
> **Fix wave 1 update (2026-08-19, security):** `POST /api/campaigns/signup` no longer trusts `siteSlug`/`campaignSlug` from the client (that was a cross-tenant lead-injection hole — see the Task 2 report's "Fix wave 1" section) — it verifies a server-signed `token` instead (`lib/campaigns/signupToken.ts`). Step 1's route (a server component, already computing `site.tenantId` + `campaign` here) must call `signCampaignSignupToken({ tenantId: site.tenantId, campaignId: campaign.id })` and pass it to `<CampaignLanding>` → `<SignupForm token={token} />`. Step 3's form POSTs `{ token, name, phone, email, message, website(honeypot) }` — drop `siteSlug`/`campaignSlug` from both the hidden form fields and the POST body entirely; they're no longer part of the accepted request shape (`lib/campaigns/signup.ts`'s `SignupInput`).
...
- [ ] **Step 1: the route** — mirror `site/[siteSlug]/page.tsx`: `resolvePublicSite({ host, siteParam: searchParams.site ?? params.siteSlug })`; if null → `notFound()`. Then `runWithTenant(site.tenantId, …)` (or the file's existing tenant-context mechanism): `getCampaignBySlug(campaignSlug)`; require an APPROVED `landing_page` asset (`listAssets` → find kind landing_page status approved) AND campaign status ∈ {ready, active} — else `notFound()`. Read branding (`getChromeLogoSrc`, `getThemeForTenant(tenantId)`, `getBrandFontIds`, `getBusinessProfile` for name/contact) + `parseLandingBody(asset.body)`. **Mint the signup token here** (server-side, this is the only place that has verified tenant+campaign): `const signupToken = signCampaignSignupToken({ tenantId: site.tenantId, campaignId: campaign.id })` (from `@/lib/campaigns/signupToken`) and pass it down: `<CampaignLanding signupToken={signupToken} …/>`. `export const dynamic = "force-dynamic"`.
- [ ] **Step 2: CampaignLanding** (server component) — a clean single-page layout using the tenant's brand tokens: header (logo), hero (headline + subhead on the accent colour), benefit bullets, `<SignupForm token={signupToken} ctaLabel={…} />`, contact (phone/business name) + a plain footer. Inline-styled from the theme accent + fonts (match the app's inline-style idiom). Must be self-contained + mobile-responsive. **No `siteSlug`/`campaignSlug` props** — the signed `token` is the only identity the form needs (they were removed in Task 2's fix wave as a cross-tenant hole).
- [ ] **Step 3: SignupForm** (client) — props: `token: string`, `ctaLabel: string`. Fields name/phone/email/optional message + a hidden honeypot (`website`, visually hidden). On submit POST JSON `{ token, name, phone, email, message, website }` to `/api/campaigns/signup` — the signed `token` carries tenant+campaign identity, so **do NOT send `siteSlug` or `campaignSlug`** (the endpoint no longer accepts them; sending them would be dead weight and re-invites the closed vuln). Disable while submitting; on `{ok:true}` show an inline thank-you ("Thanks — we'll be in touch"); on error show a message. No external deps.

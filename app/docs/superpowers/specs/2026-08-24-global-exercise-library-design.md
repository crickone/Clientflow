...
Workout items (`workout_exercises`/`workout_items`/`circuit_items`) hold a **nullable, soft** `exerciseId` (`onDelete: set null`) **plus a denormalized copy of the exercise `name`/`muscleGroups`** captured at add-time; **rendering/preview never re-queries the library** (recon §1,§4). So after exercises move to the control table:
- **Existing workouts render + edit perfectly** via their denormalized `name`/`muscleGroups` — no data migration of item FKs required.
- The old `exerciseId` values (old per-tenant ids) become **inert soft references** (never re-resolved). New picks store the control-plane id. The mixed-namespace `exerciseId` is harmless because it's never used to re-resolve a row for display — documented, not silently lost.
